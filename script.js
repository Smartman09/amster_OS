...
var pages = ["об устройстве","язык","рабочий стол","приложения","пользователь","экран","звук","библиотеки"];
for(var i in pages){
...
runnable = new Runnable() {
   run() {
        onebtn.setText(fromHtml('<b><font color="#ffffff">'+new Date().getDate()+'.'+(new Date().getMonth()+1)+'.'+new Date().getFullYear()+' </font></b>')); 
twobtn.setText(fromHtml('<b><font color="#ffffff">'+new Date().getHours()+':'+new Date().getMinutes()+':'+new Date().getSeconds()+' </font></b>')); 
...
getTextFromURL(link) {
try {
var bytearay = new java.io.ByteArrayOutputStream();
android.net.http.AndroidHttpClient.newInstance('userAgent').execute(new org.apache.http.client.methods.HttpGet(link)).getEntity().writeTo(bytearay);
bytearay.close();
return String(bytearay.toString())
} catch (e) {
print(e+' #' + e.lineNumber)
return ''
}
